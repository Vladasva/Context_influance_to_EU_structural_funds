...
def findcauses(target, cuda, epochs, kernel_size, layers,
    ...
    names = pd.read_csv(file).columns.tolist()
    ...
    for ep in range(2, epochs+1):
        ...
        if ep == 2:  # Let's start dictionary entries in the second epoch
            for idx in range(scores.numel()):
                all_scores[names[idx]] = [scores[idx].item()]
        else:  # Append scores going through epochs
            for idx in range(scores.numel()):
                all_scores[names[idx]].append(scores[idx].item())
    ...
    # Let's determine the threshold
    if ind >= 0 and ind < len(s):
        tau = s[ind]  # Let's set tau to the score at the index determined by the largest gap analysis
    else:
        tau = 1  # Let's default threshold if no suitable gap is found

    # Let's draw the attention scores graph
    plt.figure(figsize=(15, 5))

    colors = ['#e6194b', '#3cb44b', '#ffe119', '#4363d8', '#f58231', '#911eb4', '#46f0f0',
              '#f032e6', '#bcf60c', '#fabebe', '#008080', '#e6beff', '#9a6324', '#800080',
              '#800000', '#aaffc3', '#32cd32', '#ffd8b1', '#000075', '#9932cc']

    for idx, (key, value) in enumerate(all_scores.items()):
        plt.plot(value, label=f' {key}', color=colors[idx % len(colors)])

    plt.axhline(y=tau, color='r', linestyle='--', label='Slenkstis (tau)')

    plt.title('Dėmesio taškų pasiskirstimas visiems kintamiesiems ')
    plt.xlabel('Epochos')
    plt.ylabel('Dėmesio taškai')
    plt.legend()
    plt.grid(True)
    plt.show()

    # Convert the dictionary to a DataFrame and save to CSV file
    df = pd.DataFrame(dict([(k, pd.Series(v)) for k, v in all_scores.items()]))
    df.to_csv('all_scores.csv', index=False)

    return validated, causeswithdelay, realloss, scores.view(-1).cpu().detach().numpy().tolist(), all_scores
